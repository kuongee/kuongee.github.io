...
-&gt; 기존 프로젝트의 화면은 메인 뷰를 올리고 그 메인 뷰에 navigate라는 내부 함수로 화면에 해당하는 element와 js를 로드하는 것이다. 이건 새로운 vue 프로젝트에서는 알 수가 없고 제어해줄 수 있는지 파악해보았다.  
-&gt; 기존 프로젝트의 메인 뷰가 올라오기 전에 기존 프로젝트에서 navigate 에러가 발생한다. 아직 올라오지도 않은 뷰에 접근하려고 하니까  
...
알고 보니까 기존 프로젝트의 main view가 전부 로드가 되면 발생하는 이벤트가 있었다. 이 이벤트를 받을 때까지만 기다리면 되는 것이었다.
